...
- **Where we are:** Lab 4 complete on branch `lab-04`, PR open (merge = user's call). Self-measuring endpoints (tracemalloc + perf_counter): full 333 ms/14.7 MB → only() 66 ms/2.2 MB → values_list 9 ms/0.7 MB for 5k titles. Two-cost decomposition recorded (data movement vs model instantiation). Defer-touch trap: 21 queries for 20 posts, no relation involved. Tests pin SQL column lists + trap query count.
- **Next action:** After PR merges: Lab 5 — unbounded queries. `.all()` list endpoint vs paginated under locust; first real locust workout.
- **Session log:**
  ...
  - 2026-09-09 — Lab 4 built + measured (same sitting). 37×/21× time/memory spread across full→only→values; user spotted values_list memory result before explanation. **Lab 4 done**, PR open.
