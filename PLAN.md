...
- **Where we are:** Lab 17 complete on branch `lab-17`, PR pending. Arc D opened; stack grew a `worker` compose service (celery, redis broker, same dev image). Inline-vs-delay measured; queue-not-capacity backlog demonstrated with a queue-depth sampler. Tests broker-free (patch sleep + delay).
- **Next action:** After merge: Lab 18 on_commit race (create object + queue task inside a transaction → intermittent DoesNotExist in the worker; fix with transaction.on_commit; reproduce the failure, then zero failures).
- **Session log:**
