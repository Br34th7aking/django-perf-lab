...
- **Where we are:** Lab 10 complete on branch `lab-10`, README entry pending user review before PR. comment_count column + F()-based signal receivers; read 406→26 ms, writes +79% (2 statements per insert), rebuild-from-truth 1.25 s. Traps hit live: annotation-name collision broke lab 9 (renamed its annotations, payload contract kept), post_delete receiver had stray `created` arg, bulk_create bypasses signals (seed backfills; test pins the drift).
- **Next action:** After README approval → PR → Lab 11: read replica. Second postgres in compose + streaming replication + DB router (reads→replica); demo replication lag / read-your-writes anomaly. Infra-heaviest lab of Arc B.
- **Session log:**
  ...
  - 2026-09-11 — Lab 10 built + measured. Live demo of denormalization's blast radius: new column name collided with lab 9's annotations. Full trade measured (read 15×, write +79%, rebuild 1.25 s). **Lab 10 done**, awaiting README approval.
