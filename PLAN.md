...
- **Where we are:** Lab 11 complete on branch `lab-11`, README entry pending user review before PR. db_replica service (pg_basebackup clone + WAL streaming, recovery_min_apply_delay=2s for determinism), replicator role + pg_hba line on primary, ReplicaRouter in config/routers.py wired via local settings only (CI has no replica). Anomaly endpoint /labs/11/anomaly/ returns false/true. Replica boot gotchas hit: =c typo, data-dir 0700 chmod needed when bypassing the image entrypoint.
- **Next action:** After README approval → PR → Lab 12: Redis complement. Post view-counter, UPDATE-per-hit in postgres vs Redis INCR + periodic flush; throughput under locust.
- **Session log:**
  ...
  - 2026-09-11 — Lab 11 built + verified (same sitting). Real streaming replication in compose; WAL propagation demoed live (0 rows → 1 row across the 2 s window); router split verified from the shell; anomaly deterministic. **Lab 11 done**, awaiting README approval.
